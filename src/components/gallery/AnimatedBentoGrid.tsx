...
import AnimatedBentoCell, { BentoPhotoItem } from "@/components/gallery/AnimatedBentoCell";
...
export default function AnimatedBentoGrid({
  ...
  const [selectedPhoto, setSelectedPhoto] = useState<BentoPhotoItem | null>(null);
  ...
  // Distribute photos across 6 Bento cells
  const cellPools = useMemo(() => {
    if (filteredPhotos.length === 0) return [[], [], [], [], [], []];

    const pools: BentoPhotoItem[][] = [[], [], [], [], [], []];
    ...
    // Make sure each pool has at least 1 photo
  ...
  return (
    <div id={id} className="w-full relative">
      ...
      {/* 
          Mobile: grid-cols-1 lub grid-cols-2 z minimalną wysokością 
          Desktop: 4 kolumny, 3 wiersze o stałej wysokości dla idealnego Bento
      */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 md:grid-rows-3 gap-3.5 sm:gap-4 md:h-[680px] lg:h-[740px]">
        {/* Kafelek 1: Duży Hero, 2 kolumny x 2 wiersze na desktopie */}
        <AnimatedBentoCell
          photos={cellPools[0]}
        ...
        {/* Kafelek 2: Pionowy Tall, 1 kolumna x 2 wiersze na desktopie */}
        <AnimatedBentoCell
          photos={cellPools[1]}
          onPhotoClick={setSelectedPhoto}
        ...
        {/* Kafelek 3: Mały kwadrat na górze po prawej */}
        <AnimatedBentoCell
          photos={cellPools[2]}
          onPhotoClick={setSelectedPhoto}
          className="min-h-[200px] sm:min-h-[220px] md:min-h-0 sm:col-span-1 md:col-span-1 md:row-span-1"
        />

        {/* Kafelek 4: Mały kwadrat na środku po prawej */}
        <AnimatedBentoCell
          photos={cellPools[3]}
          onPhotoClick={setSelectedPhoto}
          className="min-h-[200px] sm:min-h-[220px] md:min-h-0 sm:col-span-1 md:col-span-1 md:row-span-1"
        ...
        {/* Kafelek 5: Panoramiczny kafelek na dole po lewej */}
        <AnimatedBentoCell
          photos={cellPools[4]}
          onPhotoClick={setSelectedPhoto}
          className="min-h-[200px] sm:min-h-[220px] md:min-h-0 sm:col-span-1 md:col-span-2 md:row-span-1"
        />

        {/* Kafelek 6: Panoramiczny kafelek na dole po prawej */}
        <AnimatedBentoCell
          photos={cellPools[5]}
          onPhotoClick={setSelectedPhoto}
          className="min-h-[200px] sm:min-h-[220px] md:min-h-0 sm:col-span-1 md:col-span-2 md:row-span-1"
      ...
      {/* ── LIGHTBOX MODAL (APPLE FULLSCREEN VIEW) ────────────────────── */}
      <AnimatePresence>
        {selectedPhoto && (
